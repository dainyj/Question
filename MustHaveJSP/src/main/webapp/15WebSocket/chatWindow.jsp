...
<title>웹소켓 채팅</title>
<script>
		var webSocket = new WebSocket("<%=application.getInitParameter("CHAT_ADDR")%>
	/ChatingServer");
	var chatWindow, chatMessage, chatId;

	//채팅창이 열리면 대화창, 메시지 입력창, 대화명 표시랑으로 사용할 DOM 객체 저장
	window.onload = function() {
		chat.Window = document.getElementById("chatWindow");
		chat.Message = document.getElementById("chatMessage");
		chat.Id = document.getElementById("chatId");
	...
	function sendMessage() {
		...
		chatWindow.innerHTML += "<div class='myMsg'>" + chatMessage.value
				+ "</div>"
		webSocket.send(chatId + '|' + chatMessgae.value);//서버로 전송
		chatMessgae.value = ""; // 메시지 입력창 내용 지우기
