...
//	검색 조건에 맞는 게시물의 개수를 반환합니다.
	...
	public List<MVCBoardDTO> selectListPage(Map<String, Object> map) {
		...
		// 검색 조건이 있다면 WHERE절로 추가
		if (map.get("searchWord") != null) {
			query += " WHRER " + map.get("searchField") + " LIKE '%" + map.get("searchWord") + "%' ";
		}
		...
		try {
			...
			psmt.setString(2, map.get("end").toString()); // 인파라미터 설정
...
//	게시글 데이터를 받아 DB에 추가합니다.(파일 업로드 지원).
	...
	//주어진 일련번호에 해당하는 게시물을 STO에 담아 반환합니다.
	...
	//주어진 일련번호에 해당하는 게시물의 조회수를 1 증가시십니다.
